...
const (
	// svcName is the name of qtumd service.
	svcName = "qtumdsvc"
...
// logServiceStartOfDay logs information about qtumd when the main server has
...
// qtumdService houses the main service handler which handles all service
// updates and launching qtumdMain.
type qtumdService struct{}
...
// long-running qtumdMain (which is the real meat of qtumd), handles service
// change requests, and notifies the service control manager of changes.
func (s *qtumdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start qtumdMain in a separate goroutine so the service can start
...
// installService attempts to install the qtumd service.  Typically this should
...
// removeService attempts to uninstall the qtumd service.  Typically this should
...
// startService attempts to start the qtumd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &qtumdService{})
	if err != nil {
